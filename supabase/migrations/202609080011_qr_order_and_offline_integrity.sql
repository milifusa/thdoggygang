-- Enforce operational eligibility and allow later purchases to stay linked to a booking.
alter table public.orders drop constraint if exists orders_booking_unique;
create index if not exists orders_booking_idx on public.orders(booking_id, created_at desc) where booking_id is not null;
...
begin
  ...
  if v_type='CHECK_IN' then
    if not exists(
      ...
      where bp.id=v_participant_id and b.id=v_booking_id and b.hike_id=v_hike_id and b.status='CONFIRMED'
        and (select count(*) from public.signed_waivers sw where sw.booking_id=b.id) >= (select count(*) from public.booking_participants all_bp where all_bp.booking_id=b.id)
        and exists(select 1 from public.booking_checkin_tokens t where t.booking_id=b.id and t.revoked_at is null and t.used_at is null and (t.expires_at is null or t.expires_at>now()))
    ) then raise exception 'Booking is not eligible for check-in'; end if;
    ...
    select count(*) into v_expected from public.booking_participants where booking_id=v_booking_id;
    select count(*) into v_checked from public.check_ins where booking_id=v_booking_id;
    if v_checked>=v_expected then
      update public.booking_checkin_tokens set used_at=coalesce(used_at,now()) where booking_id=v_booking_id and revoked_at is null;
    end if;
