-- Fix client booking drafts after capacity hardening.
-- The previous SECURITY INVOKER function used SELECT ... FOR UPDATE on hikes.
-- PostgreSQL applies the hike UPDATE policy to row-locking reads, so clients
-- could not see the published hike and received "Hike not found".
-- Final capacity is still serialized by the SECURITY DEFINER participant and
-- booking-status triggers installed in 202609090003.
...
begin
  ...
  -- Do not add a row-locking clause here. Authenticated clients only have
  -- SELECT access to published hikes. Capacity writes are locked by the
  -- SECURITY DEFINER triggers that run when participants/status are written.
