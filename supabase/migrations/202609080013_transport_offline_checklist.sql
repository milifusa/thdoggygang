create table if not exists public.hike_transport_departures (
  hike_id uuid primary key references public.hikes(id) on delete cascade,
...
alter table public.hike_transport_departures enable row level security;
revoke all on table public.hike_transport_departures from anon, authenticated;
grant all on table public.hike_transport_departures to service_role;

alter table public.offline_sync_operations drop constraint if exists offline_sync_operations_operation_type_check;
alter table public.offline_sync_operations add constraint offline_sync_operations_operation_type_check check (operation_type in ('CHECK_IN','PRODUCT_DELIVERY','TRANSPORT_COMPLETE','NOTE'));
...
begin
  ...
  if v_type='CHECK_IN' then
    ...
    update public.order_item_fulfillments f set status='DELIVERED',delivered_by=p_actor,delivered_at=v_client_timestamp,delivery_location=coalesce(p_operation->'payload'->>'deliveryLocation',f.delivery_location),note=coalesce(p_operation->'payload'->>'note',f.note),updated_at=now() from public.order_items oi join public.orders o on o.id=oi.order_id where f.order_item_id=(p_operation->>'orderItemId')::uuid and oi.id=f.order_item_id and o.pickup_hike_id=v_hike_id and o.fulfillment_mode='HIKE_PICKUP';
