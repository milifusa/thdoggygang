...
export function money(cents: number | null | undefined) {
  return ((cents ?? 0) / 100).toLocaleString("es-MX", { style: "currency", currency: "MXN" });
}

export function bookingStatus(status: string) {
  return ({ CONFIRMED: "CONFIRMADA", PENDING_PAYMENT: "PAGO PENDIENTE", DRAFT: "BORRADOR", CANCELLED: "CANCELADA", COMPLETED: "COMPLETADA" } as Record<string, string>)[status] ?? status;
}

export function paymentStatus(status: string) {
  return ({ PAID: "PAGADO", PENDING: "PENDIENTE", UNDER_REVIEW: "POR REVISAR", FAILED: "FALLIDO", REFUNDED: "REEMBOLSADO", CANCELLED: "CANCELADO" } as Record<string, string>)[status] ?? status;
}
