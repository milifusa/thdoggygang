...
export function EmailTemplateEditor({
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage(
      response.ok
        ? "Plantilla guardada."
        : (payload.error ?? "No pudimos guardarla."),
    );
    setSaving(false);
  };
  return (
    <details className="email-template-editor">
      <summary>
        <Mail /> EDITAR CORREO DE RECORDATORIO
      </summary>
      <form onSubmit={save}>
        <label>
          ASUNTO
        ...
        <label>
          ENCABEZADO
          <input name="heading" defaultValue={template.heading} required />
        </label>
        <label>
          MENSAJE
        ...
        <small>
          Variables disponibles: {"{nombre_cliente}"}, {"{hike}"},{" "}
          {"{fecha_hike}"}, {"{paso_pendiente}"} y {"{url_continuar}"}.
        </small>
