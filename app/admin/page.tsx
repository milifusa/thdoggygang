...
async function liveDashboard() {
  ...
  const { data: hike } = await supabase
    .from("hikes")
    .select("id, name, slug, starts_at, location_name, capacity, max_dogs")
  ...
  const { data: bookings } = hike
    ? await supabase
        .from("bookings")
        .select(
          "id, booking_number, status, total_cents, profile:profiles(first_name, last_name), booking_participants(id), booking_dogs(id), transport_reservations(id)",
        )
        .eq("hike_id", hike.id)
  ...
  return (
    <main className="admin-page">
      <aside className="admin-sidebar">
        ...
        <Link className="mode-link" href="/admin/hike-mode">
      ...
      <section className="admin-content">
        <header>
          <div>
            <p>OPERACIÓN · 08 SEP 2026</p>
            <h1>
              Buenos días,{" "}
              {session.mode === "live" ? session.profile.first_name : "Mishu"}.
            </h1>
          </div>
          <div className="admin-head-actions">
            <Link href="/admin/hikes/nuevo">＋ NUEVO HIKE</Link>
            <div className="admin-avatar">ML</div>
          </div>
        </header>
        <section className="admin-next" id="hikes">
          <div className="admin-next-image">
            <img
              ...
              alt={live?.hike?.name ?? "Sendero del Duende"}
            />
            <span>PRÓXIMA AVENTURA</span>
          </div>
          <div>
            <p>PRÓXIMO HIKE</p>
            <h2>{live?.hike?.name ?? "Sendero del Duende"}</h2>
            <span>
              {live?.hike
                ...
                : "20 SEP · 07:00 AM"}{" "}
              · {live?.hike?.location_name ?? "Cholula, Puebla"}
            </span>
            <div className="admin-stats">
              <div>
                <strong>{expected}</strong>
                <small>/ {live?.hike?.capacity ?? 40} LUGARES</small>
              </div>
              <div>
                <strong>{dogs}</strong>
              ...
              <div>
                <strong>{live?.payments.length ?? 4}</strong>
                <small>ALERTAS</small>
              </div>
            </div>
            <Link href="/admin/hike-mode" className="button button-primary">
              INICIAR MODO HIKE →
            </Link>
          </div>
        </section>
        <section className="kpi-grid">
          ...
          <article>
            ...
            <strong>{live?.bookings.length ?? 64}</strong>
            <small>
          ...
          <article>
            ...
            <strong>92%</strong>
            <small>5 pendientes de firma</small>
          </article>
          <article>
            <span>CHECK-INS</span>
            <strong>—</strong>
            <small>Abren el día del hike</small>
          </article>
        </section>
        <section className="admin-lower">
              ...
              recent.map((booking) => {
                ...
                return (
                  <div className="admin-row" key={booking.id}>
                    ...
                    <em
                      className={booking.status === "CONFIRMED" ? "paid" : ""}
                    >
                      {booking.status}
                    </em>
                    <button>→</button>
              ...
              <>
                {[
                  [
                    "TDG-1048",
                    "Mishele Lojan",
                    "3 personas · 1 perrito",
                    "PAGADO",
                  ],
                  [
                    "TDG-1047",
                    "Andrea Cruz",
                    "2 personas · 2 perritos",
                    "PENDIENTE",
                  ],
                  [
                    "TDG-1046",
                  ...
                  <div className="admin-row" key={row[0]}>
                    ...
                    <strong>{row[1]}</strong>
                    <small>{row[2]}</small>
                    <em className={row[3] === "PAGADO" ? "paid" : ""}>
                      {row[3]}
                    </em>
                    <button>→</button>
                  </div>
          ...
          <div className="alerts-panel" id="payments">
            <div className="admin-section-head">
              ...
              <span>{live?.payments.length ?? 1}</span>
            </div>
            {live?.payments.length ? (
              live.payments.map((payment) => {
                ...
                return (
                  <article key={payment.id}>
                    ...
                    <p>
                      Transferencia por $
                      {(payment.amount_cents / 100).toLocaleString("es-MX", {
                        style: "currency",
                        currency: "MXN",
                      })}
                      .
              ...
              <article>
                <strong>
                  <AlertTriangle aria-hidden="true" /> Andrea Cruz
                </strong>
                <p>Subió un comprobante para revisión.</p>
                <small>Vista demostrativa</small>
              </article>
            )}
          </div>
        </section>
