  ...
  return (
    <main className="admin-page">
      ...
      <section className="admin-content">
        ...
        <header>
          <div>
            <p>RESUMEN DE OPERACIÓN</p>
            <h1>Hola, {profile?.first_name ?? "Mishu"}.</h1>
          </div>
        ...
        <section className="kpi-grid admin-kpi-first">
          ...
          <article>
            ...
            <small>organizadas por hike</small>
          </article>
        </section>
        {nextHike ? (
          <section className="admin-next admin-next-compact">
            ...
            <div>
              ...
              <h2>{nextHike.name}</h2>
              <span>
                {adminDate(nextHike.starts_at)} · {nextHike.location_name}
              </span>
              ...
              <Link
                ...
                href={`/admin/hike-mode?hike=${nextHike.id}`}
              >
                <Bolt aria-hidden="true" /> OPERAR HIKE
              </Link>
            </div>
          </section>
        ) : (
          <section className="admin-empty-state">
            ...
            <div>
              ...
              <p>Crea el siguiente hike para comenzar.</p>
            </div>
        ...
        <section className="admin-lower admin-dashboard-lower">
          <div className="admin-table">
            ...
            <div className="admin-quick-grid">
              <Link href="/admin/hikes">
                <strong>Hikes</strong>
                <span>Edita contenido, precios y transporte.</span>
              </Link>
              <Link href="/admin/clientes">
                <strong>Clientes</strong>
                <span>Consulta expedientes y descarga responsivas.</span>
              </Link>
              <Link href="/admin/reservaciones">
                <strong>Reservaciones</strong>
                <span>Consulta personas, perritos y estatus.</span>
              </Link>
              <Link href="/admin/fotos">
                <strong>Fotografías</strong>
                <span>Carga, publica y vende recuerdos.</span>
              </Link>
              <Link href="/admin/productos">
                <strong>Productos</strong>
                <span>Administra catálogo, precios e inventario.</span>
              </Link>
              <Link href="/admin/reportes">
                <strong>Reportes</strong>
                <span>Descarga manifiestos y conciliación.</span>
              </Link>
              <Link href="/admin/configuracion-pagos">
                <strong>Configuración de pagos</strong>
                <span>Administra transferencias y Stripe.</span>
              </Link>
              <Link href="/admin/sitio">
                <strong>Sitio</strong>
                <span>Edita imágenes y textos del landing.</span>
              </Link>
            </div>
          </div>
          <div className="alerts-panel">
            <div className="admin-section-head">
              ...
              <span>{pendingPayments?.length ?? 0}</span>
            </div>
            {pendingPayments?.length ? (
              pendingPayments.map((payment) => {
                ...
                return (
                  <article key={payment.id}>
                    ...
                    <p>Transferencia por {money(payment.amount_cents)}.</p>
              ...
              <div className="admin-clear-state">
                ...
                <p>No hay pagos por revisar.</p>
              </div>
            )}
            <Link className="admin-text-link" href="/admin/pagos">
              VER TODOS LOS PAGOS
            </Link>
          </div>
        </section>
