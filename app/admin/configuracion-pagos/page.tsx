...
export default async function PaymentSettingsPage() {
  await requireStaffSession("/admin/configuracion-pagos");
  const supabase = await createSupabaseServerClient();
  const [{ data: next }, settings] = await Promise.all([
    supabase
      .from("hikes")
      .select("id")
  ...
  return (
    <main className="admin-page">
      <AdminNav active="/admin/configuracion-pagos" hikeId={next?.id} />
      <section className="admin-content">
        <AdminMobileNav />
        <header>
          <div>
            <p>COBROS Y CREDENCIALES</p>
