...
import { ArrowLeft, BusFront, Check, CircleAlert, CircleCheck, CloudDownload, Dog, House, PackageCheck, Phone, QrCode, RefreshCw, Search, Signal, SignalZero, Trash2, UserRoundCheck, X } from "lucide-react";
import type { HikeBooking, HikeModeData, OfflineOperation } from "../../lib/hike-mode-types";
import { clearHikeData, enqueueOperation, getDeviceId, getHikePackage, listOperations, removeOperation, saveHikePackage, updateOperation } from "../../lib/offline/hike-db";
...
function applyPendingOperations(source: HikeModeData, operations: OfflineOperation[]) {
  ...
  return {
    ...
    bookings: source.bookings.map((booking) => ({
      ...
      check_ins: [
        ...booking.check_ins,
        ...active.filter((operation) => operation.type === "CHECK_IN" && operation.bookingId === booking.id && operation.participantId && !booking.check_ins.some((checkin) => checkin.booking_participant_id === operation.participantId)).map((operation) => ({ id: operation.operationId, booking_participant_id: operation.participantId!, checked_in_at: operation.clientTimestamp })),
      ],
    })),
    deliveries: source.deliveries.map((delivery) => active.some((operation) => operation.type === "PRODUCT_DELIVERY" && operation.orderItemId === delivery.orderItemId) ? { ...delivery, status: "DELIVERED", deliveredAt: active.find((operation) => operation.type === "PRODUCT_DELIVERY" && operation.orderItemId === delivery.orderItemId)?.clientTimestamp ?? delivery.deliveredAt } : delivery),
    transportDeparture: active.some((operation)=>operation.type==="TRANSPORT_COMPLETE") ? { completedAt: active.find((operation)=>operation.type==="TRANSPORT_COMPLETE")?.clientTimestamp ?? null, passengerCount: source.bookings.reduce((sum,booking)=>sum+booking.transport_reservations.length,0), note: String(active.find((operation)=>operation.type==="TRANSPORT_COMPLETE")?.payload?.note??"")||null } : source.transportDeparture ?? {completedAt:null,passengerCount:0,note:null},
  };
}
...
export function HikeMode({ demo, data: initialData }: { demo: boolean; data: HikeModeData | null }) {
  ...
  const loadPackage = useCallback(async (hikeId: string, currentDeviceId: string, quiet = false) => {
    ...
    try {
      ...
      const authorization = await verifySignedPayload(payload.authorization ?? "", payload.publicKey);
      if (!authorization || authorization.purpose !== "hike-offline" || authorization.hikeId !== hikeId || authorization.deviceId !== currentDeviceId) throw new Error("El paquete offline no tiene una autorización válida para este dispositivo.");
  ...
  const syncQueue = useCallback(async () => {
    ...
    try {
      ...
      setMessage(remaining.some((operation) => operation.state === "CONFLICT") ? "Algunos cambios requieren revisión. Abre los pendientes antes de terminar el hike." : "Cambios sincronizados correctamente.");
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      setPending(operations);
      if (!cached && !navigator.onLine) setMessage("Este hike no fue preparado para trabajar sin señal. Conéctate y pulsa PREPARAR OFFLINE antes de operar.");
  ...
  const queueCheckins = async (booking: HikeBooking, participantIds: string[]) => {
    ...
    setSelected(null);
    setMessage(`${operations.length} check-in${operations.length === 1 ? "" : "s"} guardado${operations.length === 1 ? "" : "s"}.${online ? " Sincronizando…" : " Se sincronizará al recuperar la conexión."}`);
    if (online) await syncQueue();
  };
  const markDelivered = async (delivery: HikeModeData["deliveries"][number]) => {
    if (!data) return;
    const deliveryLocation=window.prompt("Lugar donde se entregó",data.hike.meeting_point??data.hike.location_name)?.trim();
    if(!deliveryLocation||!window.confirm(`¿Confirmar la entrega de ${delivery.quantity} × ${delivery.description}?`))return;
    const note=window.prompt("Nota de entrega (opcional)","")?.trim();
    const operation: OfflineOperation = { operationId: crypto.randomUUID(), hikeId: data.hike.id, bookingId: delivery.bookingId || undefined, orderItemId: delivery.orderItemId, type: "PRODUCT_DELIVERY", deviceId, clientTimestamp: new Date().toISOString(), payload: { deliveryLocation,note }, state: "PENDING" };
    ...
    await hydrateQueue(data.hike.id);
    setMessage(online ? "Entrega guardada. Sincronizando…" : "Entrega guardada en este dispositivo; se sincronizará al recuperar la conexión.");
    if (online) await syncQueue();
  };
  const markTransportComplete=async()=>{if(!data||data.transportDeparture.completedAt)return;if(!window.confirm(`Revisaste ${transport} pasajeros. ¿Marcar la salida de transporte como completa?`))return;const note=window.prompt("Nota de salida (opcional)","")?.trim()??"";const operation:OfflineOperation={operationId:crypto.randomUUID(),hikeId:data.hike.id,type:"TRANSPORT_COMPLETE",deviceId,clientTimestamp:new Date().toISOString(),payload:{passengerCount:transport,note},state:"PENDING"};await enqueueOperation(operation);setData({...data,transportDeparture:{completedAt:operation.clientTimestamp,passengerCount:transport,note:note||null}});await hydrateQueue(data.hike.id);setMessage(online?"Salida de transporte registrada. Sincronizando…":"Salida de transporte guardada en este dispositivo.");if(online)await syncQueue();};
  const deleteOfflineData = async () => {
    if (!data || !window.confirm("¿Eliminar de este dispositivo el paquete y todos los cambios pendientes de este hike?")) return;
  ...
  const openScanner = async () => {
    if (!data?.authorization) { setMessage("Primero prepara el hike para activar el escáner y la autorización operativa."); return; }
  ...
  return <main className="field-mode">
    ...
    <section className={`field-package-bar ${data.authorization ? "ready" : "not-ready"}`}><div><strong>{data.authorization ? "PAQUETE OFFLINE LISTO" : "HIKE NO PREPARADO"}</strong><span>{pending.length} cambios pendientes · última sincronización {formatTime(lastSync ?? data.preparedAt)}</span></div><div className="field-package-actions"><button disabled={preparing || !online} onClick={() => void loadPackage(data.hike.id, deviceId)}>{preparing ? <RefreshCw className="spin" /> : <CloudDownload />}{preparing ? "PREPARANDO" : data.authorization ? "ACTUALIZAR PAQUETE" : "PREPARAR OFFLINE"}</button>{data.authorization && <button className="delete-offline" onClick={() => void deleteOfflineData()}><Trash2/> ELIMINAR DATOS</button>}</div></section>
    ...
    {transport>0&&<section className="field-transport-checklist"><BusFront/><div><span>CHECKLIST DE TRANSPORTE</span><strong>{transport} pasajeros revisados</strong><small>{data.transportDeparture.completedAt?`Salida confirmada · ${formatTime(data.transportDeparture.completedAt)}`:"Confirma la salida únicamente después de revisar la lista."}</small></div><button disabled={Boolean(data.transportDeparture.completedAt)} onClick={()=>void markTransportComplete()}>{data.transportDeparture.completedAt?<><CircleCheck/> TRANSPORTE COMPLETO</>:"MARCAR TRANSPORTE COMPLETO"}</button></section>}
    ...
    <button className="field-scan-sticky" disabled={!data.authorization} onClick={() => void openScanner()}><QrCode /><span>{data.authorization ? "ESCANEAR QR" : "PREPARA EL HIKE PARA ESCANEAR"}</span></button>
    ...
    {selected && <div className="field-modal"><section className="field-booking-sheet"><header><button onClick={() => setSelected(null)}><X /></button><div><span>{selected.booking_number} · PAGO CONFIRMADO</span><h2>{bookingName(selected)}</h2></div></header>{selected.transport_reservations.length>0&&<div className="field-transport-summary"><BusFront/><strong>{selected.transport_reservations.length} lugares de transporte</strong></div>}{hasDogAlert(selected) && <div className="field-dog-alert"><CircleAlert /><div><strong>INDICACIONES DE PERRITOS</strong>{selected.booking_dogs.filter((dog) => dog.snapshot.reactivity || dog.snapshot.medical_conditions || dog.snapshot.medications || dog.snapshot.notes).map((dog) => <p key={dog.id}><b>{dog.snapshot.name ?? "Perrito"}:</b> {[dog.snapshot.reactivity,dog.snapshot.medical_conditions,dog.snapshot.medications,dog.snapshot.notes].filter(Boolean).join(" · ")}</p>)}</div></div>}<div className="field-people"><span>ASISTENTES</span>{selected.booking_participants.map((person) => <article key={person.id}><span>{checkedIds.has(person.id) ? <CircleCheck /> : <span />}</span><div><strong>{personName(person)}</strong><small>{person.snapshot.is_minor ? "Menor" : "Adulto"}{selected.signed_waivers.some((waiver) => waiver.booking_participant_id === person.id) ? " · Responsiva firmada" : " · Responsiva pendiente"}</small></div></article>)}</div>{selected.booking_dogs.length>0&&<div className="field-booking-dogs"><span>PERRITOS</span><strong>{selected.booking_dogs.map((dog)=>dog.snapshot.name??"Perrito").join(" · ")}</strong></div>}<div className="field-sheet-actions"><button onClick={() => void queueCheckins(selected, selected.booking_participants.map((person) => person.id))}><Check /> CONFIRMAR CHECK-IN PENDIENTE</button>{selected.booking_participants.some((person) => person.snapshot.emergency_contact_phone) && <a href={`tel:${selected.booking_participants.find((person) => person.snapshot.emergency_contact_phone)?.snapshot.emergency_contact_phone}`}><Phone /> CONTACTO DE EMERGENCIA</a>}</div></section></div>}
