        ...
        <section className="admin-panel admin-module-panel">
          <div className="admin-section-head">
            <div>
              ...
              <h2>{clients?.length ?? 0} clientes</h2>
            </div>
          ...
          <div className="admin-client-list">
            {clients?.map((client) => {
              ...
              const waiverCount = new Set(
                bookings.flatMap((booking) =>
                  booking.signed_waivers.map((waiver) => waiver.pdf_path),
                ),
              ).size;
              return (
                <details className="admin-client-card" key={client.id}>
                  <summary>
                    ...
                    <span>{bookings.length} reservaciones</span>
                    <span>{waiverCount} responsivas</span>
                      ...
                      {bookings.length ? (
                        bookings.map((booking) => {
                          const hike = Array.isArray(booking.hike)
                            ? booking.hike[0]
                            : booking.hike;
                          const uniqueWaivers = Array.from(
                            new Map(
                              booking.signed_waivers.map((waiver) => [
                                waiver.pdf_path,
                                waiver,
                              ]),
                            ).values(),
                          ...
                          return (
                            <article key={booking.id}>
                              <div>
                                ...
                                <small>
                                  {adminDate(booking.created_at)} ·{" "}
                                  {money(booking.total_cents)} ·{" "}
                                  {bookingStatus(booking.status)}
                                </small>
                              </div>
                              <div className="admin-waiver-files">
                                {uniqueWaivers.length ? (
                                  uniqueWaivers.map((waiver) => (
                                    <Link
                                      href={`/api/admin/waivers/${waiver.id}/download`}
                                      key={waiver.id}
                                    >
                                      <FileCheck2 aria-hidden="true" />
                                      <span>
                                        <strong>Responsiva firmada</strong>
                                        <small>
                                          {adminDate(waiver.signed_at)}
                                        </small>
                                      </span>
                                  ...
                                  <span className="admin-no-waiver">
                                    Sin responsiva firmada
                                  </span>
                                )}
                              </div>
                            </article>
