...
async function navigationForCurrentRole() {
  ...
  return {
    role: guide ? "GUÍA" : "ADMIN",
    sections: guide
      ? sections.filter((item) => item.href === "/admin/hikes")
      : sections,
  };
}
...
export async function AdminNav({
  ...
  return (
    <aside className="admin-sidebar">
      ...
      <Link
        className="mode-link"
        href={hikeId ? `/admin/hike-mode?hike=${hikeId}` : "/admin/hike-mode"}
      >
    ...
    <nav className="admin-mobile-nav" aria-label="Navegación administrativa">
      <details>
        <summary>
          ...
          <span>MENÚ DEL ADMINISTRADOR</span>
