...
export default async function BookingPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ booking?:string; step?:string }> }) {
  ...
  if (context.mode === 'live' && requested.booking) {
    ...
    if (booking && hike?.slug === slug) {
      ...
      resume={ bookingId:booking.id, step:Math.max(0,stepNames.indexOf(requested.step || booking.current_step)), personIds:booking.booking_participants.map((participant)=>participant.person_profile_id).filter(Boolean) as string[], dogIds:booking.booking_dogs.map((dog)=>dog.dog_id).filter(Boolean) as string[], transportPersonIds:booking.transport_reservations.map((reservation)=>participantMap.get(reservation.booking_participant_id)).filter(Boolean) as string[], productSelections:booking.booking_product_selections.map((item)=>({productId:item.product_id,variant:item.variant,quantity:item.quantity})), waiverSigned:booking.signed_waivers.length>0, creditAppliedCents:booking.credit_applied_cents };
    }
  }
  const [bankTransfer, stripeSecret, creditBalanceCents] = await Promise.all([getBankTransferConfig(), getStripeSecretKey(), context.profileId ? getMemberCreditBalance(context.profileId) : Promise.resolve(0)]);
  return <BookingWizard adventure={adventure} context={context} products={products.filter((product) => product.pickupEnabled)} cardPaymentsEnabled={Boolean(stripeSecret)} bankTransfer={bankTransfer} creditBalanceCents={creditBalanceCents + (resume?.creditAppliedCents ?? 0)} resume={resume} />;
}
