...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!z.string().uuid().safeParse(id).success) return Response.json({ error: 'Perrito inválido.' }, { status: 400 });
  const parsed = dogSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return Response.json({ error: 'Revisa los datos de tu perrito.' }, { status: 400 });
  const supabase = await createSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return Response.json({ error: 'Inicia sesión para editar perritos.' }, { status: 401 });
  const { data: profile } = await supabase.from('profiles').select('id').eq('auth_user_id', user.id).single();
  if (!profile) return Response.json({ error: 'No encontramos tu perfil.' }, { status: 404 });
  if (parsed.data.photoPath && !parsed.data.photoPath.startsWith(`${profile.id}/`)) {
    return Response.json({ error: 'La foto no pertenece a tu cuenta.' }, { status: 400 });
  }
  ...
  if (error || !data) return Response.json({ error: error?.message ?? 'No encontramos a ese perrito.' }, { status: 400 });
