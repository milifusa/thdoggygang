...
export async function POST(request: Request) {
  ...
  const { data: booking } = await supabase.from('bookings').select('id, booking_number, total_cents, currency, status, hike:hikes(name, slug), booking_participants(id), booking_product_selections(product_id,variant,quantity,unit_price_cents,product:products(name))').eq('id', parsed.data.bookingId).single();
  ...
  const { data: existingOrder } = await service.from('orders').select('id, order_number').eq('booking_id', booking.id).is('fulfillment_mode',null).limit(1).maybeSingle();
  ...
  const checkoutLines = [{ name: hike?.name ?? 'Aventura The Doggy Gang', amount: booking.total_cents - booking.booking_product_selections.reduce((sum, item) => sum + item.quantity * item.unit_price_cents, 0), quantity: 1 }, ...booking.booking_product_selections.map((item) => { const product = Array.isArray(item.product) ? item.product[0] : item.product; return { name: `${product?.name ?? 'Producto'}${item.variant ? ` · ${item.variant}` : ''}`, amount: item.unit_price_cents, quantity: item.quantity }; })];
  checkoutLines.forEach((line, index) => { form.set(`line_items[${index}][price_data][currency]`, booking.currency.toLowerCase()); form.set(`line_items[${index}][price_data][unit_amount]`, String(line.amount)); form.set(`line_items[${index}][price_data][product_data][name]`, line.name); form.set(`line_items[${index}][quantity]`, String(line.quantity)); });
