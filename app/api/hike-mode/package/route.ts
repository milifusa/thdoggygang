...
import type { HikeBooking, HikeDelivery, HikeModeData } from "../../../lib/hike-mode-types";
import { hashSignedToken, signPayload, type OfflineAuthorizationPayload } from "../../../lib/security/signed-token";
...
export async function GET(request: Request) {
  ...
  const parsed = querySchema.safeParse({ hike: url.searchParams.get("hike"), device: url.searchParams.get("device") });
  if (!parsed.success) return Response.json({ error: "Solicitud inválida." }, { status: 400 });

  const userClient = await createSupabaseServerClient();
  const { data: { user } } = await userClient.auth.getUser();
  if (!user) return Response.json({ error: "No autorizado." }, { status: 401 });
  const { data: staff } = await userClient.from("profiles").select("id,role,active").eq("auth_user_id", user.id).single();
  if (!staff?.active || !["ADMIN", "GUIDE"].includes(staff.role)) return Response.json({ error: "No autorizado." }, { status: 403 });
  if (staff.role === "GUIDE") {
    const { data: assignment } = await userClient.from("guide_hikes").select("hike_id").eq("hike_id", parsed.data.hike).eq("profile_id", staff.id).maybeSingle();
  ...
  for (const order of rawOrders ?? []) {
    for (const item of order.order_items ?? []) {
      ...
      const fulfillment = Array.isArray(item.order_item_fulfillments) ? item.order_item_fulfillments[0] : item.order_item_fulfillments;
      deliveries.push({
  ...
  const expiry = new Date(new Date(hike.starts_at).getTime() + 72 * 60 * 60 * 1000);
