...
export async function POST(request: Request) {
  ...
  const [{ data: profile }, { data: hike }] = await Promise.all([
    ...
    supabase.from('hikes').select('id').eq('slug', parsed.data.hikeSlug).is('deleted_at', null).single(),
  ]);
  if (!profile || !hike) return Response.json({ error: 'No encontramos tu perfil o el hike.' }, { status: 404 });
  let effectiveBookingId = parsed.data.bookingId;
  if (!effectiveBookingId) {
    const { data: active } = await supabase.from('bookings').select('id,status').eq('profile_id', profile.id).eq('hike_id', hike.id).in('status', ['DRAFT','PENDING_PAYMENT','CONFIRMED']).order('updated_at', { ascending: false }).limit(1).maybeSingle();
    if (active?.status === 'CONFIRMED') return Response.json({ error: 'Ya tienes una reservación confirmada para este hike.', code: 'ALREADY_CONFIRMED' }, { status: 409 });
    if (active?.status === 'PENDING_PAYMENT') return Response.json({ error: 'Ya tienes una reservación con pago pendiente para este hike.', code: 'PAYMENT_PENDING' }, { status: 409 });
  ...
  await supabase.from('booking_product_selections').delete().eq('booking_id', data);
  if (productIds.length) {
    const { data: products } = await supabase.from('products').select('id,name,price_cents,stock,variants,pickup_enabled').in('id', productIds).eq('active', true).is('deleted_at', null);
    if (!products || products.length !== productIds.length) return Response.json({ error: 'Uno de los productos ya no está disponible.' }, { status: 409 });
    const rows = [];
    for (const selection of selections) {
      const product = products.find((item) => item.id === selection.productId);
      if (!product || !product.pickup_enabled || selection.quantity > product.stock) return Response.json({ error: 'Revisa la disponibilidad de los productos.' }, { status: 409 });
      if (product.variants?.length && !product.variants.includes(selection.variant)) return Response.json({ error: `Selecciona una variante válida para ${product.name}.` }, { status: 400 });
      productSubtotal += product.price_cents * selection.quantity;
      rows.push({ booking_id: data, product_id: product.id, variant: selection.variant, quantity: selection.quantity, unit_price_cents: product.price_cents });
    }
    const { error: selectionError } = await supabase.from('booking_product_selections').insert(rows);
    if (selectionError) return Response.json({ error: selectionError.message }, { status: 400 });
  }
