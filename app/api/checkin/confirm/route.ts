...
export async function POST(request: Request) {
  ...
  if (!profile?.active || !['GUIDE', 'ADMIN'].includes(profile.role)) return Response.json({ error: 'Perfil no autorizado' }, { status: 403 });
  const { data: booking } = await supabase.from('bookings').select('id, hike_id,status,booking_participants(id),signed_waivers(id,booking_participant_id)').eq('id', parsed.data.bookingId).eq('hike_id', parsed.data.hikeId).single();
  const allowedParticipantIds = new Set(booking?.booking_participants.map((participant) => participant.id) ?? []);
  if (!booking || booking.status !== 'CONFIRMED' || booking.signed_waivers.length < booking.booking_participants.length || parsed.data.participantIds.some((id) => !allowedParticipantIds.has(id))) return Response.json({ error: 'La reservación debe estar confirmada y tener todas sus responsivas.' }, { status: 403 });
  ...
  const { count: checkedCount } = await service.from('check_ins').select('id', { count: 'exact', head: true }).eq('booking_id', parsed.data.bookingId);
  if ((checkedCount ?? 0) >= booking.booking_participants.length) await service.from('booking_checkin_tokens').update({ used_at: new Date().toISOString() }).eq('booking_id', parsed.data.bookingId).is('revoked_at', null).is('used_at', null);
