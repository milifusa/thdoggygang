...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return Response.json({ error: "QR inválido" }, { status: 400 });
  ...
  if (signedPayload && signedPayload.purpose !== "checkin")
    return Response.json({ error: "QR inválido" }, { status: 400 });
  ...
  if (signedPayload?.purpose === "checkin")
    query = query
      .eq("token_id", signedPayload.tokenId)
      .eq("booking_id", signedPayload.bookingId);
  ...
  if (
    ...
    return Response.json(
      { error: "La reservación no está habilitada para check-in." },
      { status: 409 },
    );
  if (
    signedPayload?.purpose === "checkin" &&
    booking?.hike_id !== signedPayload.hikeId
  )
    return Response.json(
      { error: "Este QR pertenece a otro hike." },
      { status: 403 },
    );
