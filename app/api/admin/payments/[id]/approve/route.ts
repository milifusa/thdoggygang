...
export async function POST(
  ...
  const { data: payment } = await service
    .from("payments")
    .select("id, order_id, status, order:orders(booking_id)")
    .eq("id", id)
    .single();
  if (!payment || payment.status !== "UNDER_REVIEW")
    return Response.json(
      { error: "Este pago ya fue revisado o no existe." },
      { status: 409 },
    );
  ...
  await service
    ...
    .update({ reviewed_by: admin.id, reviewed_at: new Date().toISOString() })
    .eq("payment_id", payment.id);
  await service
    .from("orders")
    .update({ status: "PAID" })
    .eq("id", payment.order_id);
  await service.rpc("commit_product_inventory", {
  ...
  await service
    ...
    .insert({
      ...
      action: "TRANSFER_PAYMENT_APPROVED",
      entity_type: "payment",
