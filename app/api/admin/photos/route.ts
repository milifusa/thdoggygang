...
export async function POST(request: Request) {
  ...
  if (!files.length || files.length > 100) return Response.json({ error: "Selecciona entre 1 y 100 fotos." }, { status: 400 });
  ...
  const { data: gallery, error: galleryError } = await service.from("hike_galleries").upsert({ hike_id: hike.id, title: `Fotos · ${hike.name}` }, { onConflict: "hike_id" }).select("id").single();
  if (galleryError || !gallery) return Response.json({ error: galleryError?.message ?? "No pudimos crear la galería." }, { status: 500 });
  const saved = [];
  const logo = await readFile(path.join(process.cwd(), "public/brand/logo-circular-blue.png"));
  for (const file of files) {
    try {
      const original = Buffer.from(await file.arrayBuffer());
      const image = sharp(original, { failOn: "error" }).rotate();
      const metadata = await image.metadata();
      const preview = await image.clone().resize({ width: 1600, height: 1600, fit: "inside", withoutEnlargement: true }).jpeg({ quality: 82 }).toBuffer();
      const thumbnail = await image.clone().resize({ width: 520, height: 520, fit: "inside", withoutEnlargement: true }).jpeg({ quality: 76 }).toBuffer();
      const previewInfo = await sharp(preview).metadata();
      const watermarkWidth = Math.max(220, Math.round((previewInfo.width ?? 1200) * .34));
      const watermark = await sharp(logo).resize({ width: watermarkWidth }).ensureAlpha(.58).png().toBuffer();
      const watermarked = await sharp(preview).composite([{ input: watermark, gravity: "centre", blend: "over" }]).jpeg({ quality: 82 }).toBuffer();
      ...
      await Promise.all([
        ...
        service.storage.from("hike-previews").upload(thumbnailPath, thumbnail, { contentType: "image/jpeg", upsert: false }),
      ...
      const { data, error } = await service.from("photos").insert({ id, gallery_id: gallery.id, original_path: originalPath, thumbnail_path: thumbnailPath, preview_path: previewPath, watermarked_path: previewPath, access: access.data, price_cents: access.data === "PAID" ? priceCents : 0, width: metadata.width ?? null, height: metadata.height ?? null, title: file.name.replace(/\.[^.]+$/, ""), processing_status:"READY" }).select("id").single();
