...
export async function PATCH(
  ...
  if (!parsed.success)
    return Response.json(
      { error: "Revisa los datos de la persona." },
      { status: 400 },
    );
  ...
  if (parsed.data.guardianPersonId === id)
    return Response.json(
      { error: "Una persona no puede ser su propio responsable." },
      { status: 400 },
    );
  ...
  if (!user)
    return Response.json(
      { error: "Inicia sesión para editar personas." },
      { status: 401 },
    );
  ...
  if (error || !data)
    return Response.json(
      { error: error?.message ?? "No encontramos a esa persona." },
      { status: 400 },
    );
...
export async function DELETE(
  ...
  if (!user)
    return Response.json(
      { error: "Inicia sesión para administrar tu manada." },
      { status: 401 },
    );
  const { data: profile } = await supabase
    .from("profiles")
  ...
  if (dependent)
    return Response.json(
      {
        error:
          "Antes de eliminarla, asigna otro responsable a los menores bajo su tutela.",
      },
      { status: 409 },
    );
  const { data, error } = await supabase
