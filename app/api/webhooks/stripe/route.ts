...
async function verifyStripeSignature(body: string, header: string, secret: string) {
  const parsed = parseSignature(header); if (!parsed.t || !parsed.v1 || Math.abs(Date.now() / 1000 - Number(parsed.t)) > 300) return false;
  const key = await crypto.subtle.importKey('raw', new TextEncoder().encode(secret), { name: 'HMAC', hash: 'SHA-256' }, false, ['verify']);
  const signature = Uint8Array.from(parsed.v1.match(/.{2}/g) ?? [], (pair) => Number.parseInt(pair, 16));
  return crypto.subtle.verify('HMAC', key, signature, new TextEncoder().encode(`${parsed.t}.${body}`));
}
...
export async function POST(request: Request) {
  ...
  const { error: eventError } = await service.from('payment_webhook_events').insert({ provider: 'stripe', event_id: event.id, event_type: event.type, payload: event });
