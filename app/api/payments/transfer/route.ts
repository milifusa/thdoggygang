...
export async function POST(request: Request) {
  ...
  const upload = await service.storage.from('payment-receipts').upload(storagePath, await receipt.arrayBuffer(), { contentType: receipt.type, upsert: false });
  if (upload.error) return Response.json({ error: 'No pudimos guardar el comprobante.' }, { status: 500 });
  const payment = await service.from('payments').insert({ order_id: order.id, provider: 'bank_transfer', provider_payment_id: `transfer:${crypto.randomUUID()}`, method: 'TRANSFER', status: 'UNDER_REVIEW', amount_cents: booking.total_cents }).select('id').single();
  if (payment.error || !payment.data) return Response.json({ error: 'No pudimos registrar el pago.' }, { status: 500 });
  await service.from('payment_receipts').insert({ payment_id: payment.data.id, uploaded_by: booking.profile_id, storage_path: storagePath });
  await service.from('orders').update({ status: 'UNDER_REVIEW' }).eq('id', order.id);
  await service.from('bookings').update({ status: 'PENDING_PAYMENT' }).eq('id', booking.id);
  await service.from('audit_logs').insert({ actor_profile_id: booking.profile_id, action: 'PAYMENT_RECEIPT_UPLOADED', entity_type: 'booking', entity_id: booking.id, metadata: { storage_path: storagePath } });
