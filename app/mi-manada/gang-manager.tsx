...
function EmptyCard({ label, action, onClick }: { label: string; action: string; onClick: () => void }) {
  return <div className="gang-empty"><span><Plus aria-hidden="true" /></span><p>{label}</p><button type="button" onClick={onClick}>{action} <ArrowRight aria-hidden="true" /></button></div>;
}
...
function PersonDialog({ record, onClose, onSaved }: { record?: PersonRecord; onClose: () => void; onSaved: () => void }) {
  ...
  return <Dialog title={record ? 'Edita a tu acompañante' : 'Agrega a una persona'} intro="Guarda sus datos una vez y selecciónala fácilmente en futuras aventuras." onClose={onClose}>
    <form className="gang-form" onSubmit={submit}>
      ...
      <div className="gang-form-actions"><button type="button" onClick={onClose}>CANCELAR</button><button className="button button-primary" disabled={saving}>{saving ? 'GUARDANDO…' : <>GUARDAR PERSONA <ArrowRight aria-hidden="true" /></>}</button></div>
    </form>
  </Dialog>;
}
...
function DogDialog({ profileId, record, onClose, onSaved }: { profileId: string; record?: DogRecord; onClose: () => void; onSaved: () => void }) {
  ...
  return <Dialog title={record ? `Edita a ${record.name}` : 'Agrega a tu perrito'} intro="Esta información ayuda al equipo a preparar una aventura segura y feliz." onClose={onClose}>
    <form className="gang-form" onSubmit={submit}>
      ...
      {error && <p className="gang-form-error" role="alert">{error}</p>}
      <div className="gang-form-actions"><button type="button" onClick={onClose}>CANCELAR</button><button className="button button-primary" disabled={saving}>{saving ? 'GUARDANDO…' : <>GUARDAR PERRITO <ArrowRight aria-hidden="true" /></>}</button></div>
    </form>
  </Dialog>;
}
...
function Dialog({ title, intro, onClose, children }: { title: string; intro: string; onClose: () => void; children: React.ReactNode }) {
  ...
  return <div className="gang-dialog-backdrop" role="presentation"><section className="gang-dialog" role="dialog" aria-modal="true" aria-labelledby="gang-dialog-title"><header><div><p>MI MANADA</p><h2 id="gang-dialog-title">{title}</h2><span>{intro}</span></div><button type="button" onClick={onClose} aria-label="Cerrar"><X aria-hidden="true" /></button></header><div className="gang-scroll-cue">FORMULARIO · DESLIZA PARA VER TODOS LOS DATOS</div>{children}</section></div>;
}
