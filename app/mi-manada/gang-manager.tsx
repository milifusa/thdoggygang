...
type DialogState =
  | { kind: "person"; record?: PersonRecord }
  | { kind: "dog"; record?: DogRecord }
  | null;
...
export function GangManager({
  ...
  return (
    <>
      {notice && (
        <div className="gang-notice" role="status">
          LISTO · {notice}
        </div>
      )}
      <div className="account-heading" id="personas">
        <div>
          <p className="eyebrow">QUIÉNES CAMINAN CONTIGO</p>
          <h2>Mi manada</h2>
        </div>
        <button type="button" onClick={() => setDialog({ kind: "person" })}>
          <Plus aria-hidden="true" /> AGREGAR PERSONA
        </button>
      </div>
      {people.length ? (
        ...
        <EmptyCard
          label="Todavía no has agregado personas a tu manada."
          action="AGREGAR PRIMERA PERSONA"
          onClick={() => setDialog({ kind: "person" })}
        />
      )}

      <div className="account-heading" id="perritos">
        <div>
          <p className="eyebrow">EXPLORADORES DE CUATRO PATAS</p>
          <h2>Mis perritos</h2>
        </div>
        <button type="button" onClick={() => setDialog({ kind: "dog" })}>
      ...
      {dogs.length ? (
        <div className="dog-card-grid">
          {dogs.map((dog) => (
            <article className="dog-profile-card" key={dog.id}>
              <img src={dog.photoUrl || fallbackDogPhoto} alt={dog.name} />
              <div>
                <span>MIEMBRO DE LA MANADA</span>
                <h2>{dog.name}</h2>
                <p>
                  {dog.breed || "Perrito"}
                  {age(dog.birthDate) !== null
                    ? ` · ${age(dog.birthDate)} años`
                    : ""}{" "}
                  · {sexLabel[dog.sex]}
                </p>
                <dl>
                  ...
                  <div>
                    <dt>SOCIAL</dt>
                    <dd>{dog.sociability || "—"}</dd>
                  </div>
                  <div>
                    <dt>TAMAÑO</dt>
                    <dd>{sizeLabel[dog.size]}</dd>
                  </div>
                </dl>
                <button
                  type="button"
                  onClick={() => setDialog({ kind: "dog", record: dog })}
                >
                  EDITAR PERFIL <ArrowRight aria-hidden="true" />
                </button>
              </div>
        ...
        <EmptyCard
          label="Agrega a tu compañero de cuatro patas para incluirlo en una aventura."
          action="AGREGAR MI PERRITO"
          onClick={() => setDialog({ kind: "dog" })}
        />
      )}
      ...
      {dialog?.kind === "dog" && (
        <DogDialog
          ...
          onSaved={() =>
            saved(
              dialog.record
                ? "Perfil de tu perrito actualizado."
                : "Perrito agregado a tu manada.",
            )
          }
        />
      )}
    ...
    <label>
      ...
      <span className="mexico-phone">
        ...
        <input
          ...
          value={digits}
          onChange={(event) =>
            setDigits(mexicoNationalDigits(event.target.value))
          }
...
function PersonDialog({
  ...
  const remove = async () => {
    if (
      !record ||
      !window.confirm(
        `¿Quitar a ${record.firstName} de tu manada? Su historial permanecerá en reservaciones anteriores.`,
      )
    )
      return;
    setSaving(true);
    const response = await fetch(`/api/people/${record.id}`, {
      method: "DELETE",
    });
    const result = (await response.json()) as { error?: string };
    if (response.ok) onSaved();
    else setError(result.error ?? "No pudimos eliminar la persona.");
    setSaving(false);
  };
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      isMinor: minor,
      guardianPersonId: minor ? data.get("guardianPersonId") : "",
    };
    try {
      const response = await fetch(
        record ? `/api/people/${record.id}` : "/api/people",
        {
      ...
      const result = (await response.json()) as { error?: string };
      if (!response.ok)
        throw new Error(result.error ?? "No pudimos guardar la persona.");
      onSaved();
    } catch (cause) {
      setError(
        cause instanceof Error
          ? cause.message
          : "No pudimos guardar la persona.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    ...
    >
      <form className="gang-form" onSubmit={submit}>
          ...
          {minor && (
            <label>
              ...
              <select
                ...
                defaultValue={record?.guardianPersonId}
              >
                <option value="">Selecciona un responsable</option>
                {adultGuardians.map((person) => (
              ...
              <small>
                La persona responsable también deberá asistir y firmar.
              </small>
            </label>
          )}
        </div>
        <div className="gang-subsection">
        ...
        {error && (
          <p className="gang-form-error" role="alert">
            {error}
          </p>
        )}
        <div className="gang-form-actions">
          {record && (
            <button
          ...
          <button className="button button-primary" disabled={saving}>
            {saving ? (
              ...
              <>
                GUARDAR PERSONA <ArrowRight aria-hidden="true" />
              </>
            )}
          </button>
        </div>
...
function DogDialog({
  ...
  const remove = async () => {
    if (
      !record ||
      !window.confirm(
        `¿Quitar a ${record.name} de tu manada? Su historial permanecerá en reservaciones anteriores.`,
      )
    )
      return;
    setSaving(true);
    const response = await fetch(`/api/dogs/${record.id}`, {
      method: "DELETE",
    });
    const result = (await response.json()) as { error?: string };
    if (response.ok) onSaved();
    else setError(result.error ?? "No pudimos eliminar a tu perrito.");
    setSaving(false);
  };
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    ...
    try {
      ...
      const result = (await response.json()) as { error?: string };
      if (!response.ok)
        throw new Error(result.error ?? "No pudimos guardar a tu perrito.");
      onSaved();
    } catch (cause) {
      setError(
        cause instanceof Error
          ? cause.message
          : "No pudimos guardar a tu perrito.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Dialog
      title={record ? `Edita a ${record.name}` : "Agrega a tu perrito"}
      intro="Esta información ayuda al equipo a preparar una aventura segura y feliz."
      onClose={onClose}
    >
      <form className="gang-form" onSubmit={submit}>
        <label className="dog-photo-field">
          <span>
            {existingPhoto ? (
              <img src={existingPhoto} alt="Foto actual" />
            ) : (
              "FOTO"
            )}
          </span>
          <b>
            {photoName || (existingPhoto ? "CAMBIAR FOTO" : "AGREGAR FOTO")}
          </b>
        ...
        <div className="gang-form-grid">
          ...
          <label>
            ...
            <input
              name="breed"
              defaultValue={record?.breed}
              placeholder="Mestizo, Westie…"
            />
          </label>
          <label>
            FECHA DE NACIMIENTO
          ...
          <label>
            SEXO
            <select name="sex" defaultValue={record?.sex ?? "UNKNOWN"}>
          ...
          <label>
            TAMAÑO
            <select name="size" defaultValue={record?.size ?? "MEDIUM"}>
          ...
          <label>
            ...
            <select
              name="sterilized"
              defaultValue={
                record?.sterilized === null || record?.sterilized === undefined
                  ? ""
                  : String(record.sterilized)
              }
            >
          ...
          <label>
            ...
            <input
              name="sociability"
              defaultValue={record?.sociability}
              placeholder="Muy sociable, tímido…"
            />
          </label>
          <label>
            REACTIVIDAD
          ...
          <label className="full-field">
            ...
            <textarea
              ...
              defaultValue={record?.medicalConditions}
            />
          </label>
          <label className="full-field">
            MEDICAMENTOS
            <textarea name="medications" defaultValue={record?.medications} />
          </label>
          <label className="full-field">
            NOTAS PARA LOS GUÍAS
        ...
        <div className="gang-form-actions">
          ...
          <button className="button button-primary" disabled={saving}>
            {saving ? (
              ...
              <>
                GUARDAR PERRITO <ArrowRight aria-hidden="true" />
              </>
            )}
          </button>
        </div>
...
function Dialog({
  ...
  return (
    <div className="gang-dialog-backdrop" role="presentation">
      <section
        className="gang-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="gang-dialog-title"
      >
        <header>
          <div>
            <p>MI MANADA</p>
            <h2 id="gang-dialog-title">{title}</h2>
            <span>{intro}</span>
          </div>
        ...
        <div className="gang-scroll-cue">
          FORMULARIO · DESLIZA PARA VER TODOS LOS DATOS
        </div>
        {children}
      </section>
    </div>
