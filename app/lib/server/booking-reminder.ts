...
export async function sendBookingReminder(bookingId: string, actorProfileId: string) {
  ...
  const subject = template?.subject ?? "Tu aventura sigue esperando";
  const heading = template?.heading ?? "Termina tu reservación";
  const body = template?.body ?? "Guardamos tu avance para que puedas retomar tu reservación.";
  const buttonLabel = template?.button_label ?? "CONTINUAR RESERVACIÓN";
