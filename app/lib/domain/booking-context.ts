  ...
  dogs: [
    {
      id: "mona",
      name: "Mona",
      detail: "Westie · 5 años · 3 aventuras",
      image: "/brand/profile-trail-sun.png",
    },
    {
      id: "bruno",
      name: "Bruno",
      detail: "Border Collie · 3 años · 1 aventura",
      image: "/brand/profile-trail-sun.png",
    },
  ],
};
...
export async function loadBookingContext(): Promise<BookingContext> {
  ...
  const [{ data: people }, { data: dogs }] = await Promise.all([
    ...
    supabase
      .from("dogs")
      .select("id, name, breed, birth_date, photo_path")
      .eq("owner_profile_id", profile.id)
      .is("deleted_at", null)
      .order("created_at"),
  ]);
  const age = (birthDate: string | null) =>
    birthDate
      ...
      : null;
  const dogOptions = await Promise.all(
    (dogs ?? []).map(async (dog) => {
      ...
      return {
        id: dog.id,
        name: dog.name,
        detail: `${dog.breed ?? "Perrito"}${age(dog.birth_date) ? ` · ${age(dog.birth_date)} años` : ""}`,
        image,
      };
    }),
  );
  return {
    ...
    people: (people ?? []).map((person, index) => ({
      ...
      detail: `${index === 0 ? "Titular" : "Acompañante"} · ${person.is_minor ? "Menor" : "Adulto"}`,
      initials:
