...
export const defaultLoginContent: LoginContent = {
  quote: "Cada aventura empieza con un sí.",
  quoteVisible: true,
  eyebrow: "BIENVENIDO A TU MANADA",
  ...
  emailCta: "ENVIAR ACCESO",
  phoneLabel: "TU TELÉFONO",
  phonePlaceholder: "2220000000",
  phoneCta: "ENVIAR CÓDIGO",
  codeLabel: "CÓDIGO DE 6 DÍGITOS",
  verifyCta: "VERIFICAR Y ENTRAR",
  legalText: "Al continuar aceptas nuestros",
...
export async function getLoginSettings() {
  ...
  const { data } = await client
    .from("site_content")
    .select("content,hero_image_path,mobile_image_path,image_metadata")
    .eq("id", "login")
    .maybeSingle();
  const raw =
    data?.content && typeof data.content === "object"
      ? (data.content as Partial<LoginContent>)
      : {};
  ...
  return {
    content,
    desktopImage,
    mobileImage: siteAssetUrl(data?.mobile_image_path) || desktopImage,
